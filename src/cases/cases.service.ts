...
import { criminals } from './schema/criminals.model';

@Injectable()
export class CasesService {
  constructor(
    @InjectModel(cases.name) private readonly caseModel: Model<caseDoc>,
    @InjectModel(criminals.name) private readonly criminalModel: Model<caseDoc>,
  ) { }
    
    async caseRegister(dto: caseDTo,res) {
        await this.caseModel.findOne({ caseNo: dto.caseNo }).exec()
